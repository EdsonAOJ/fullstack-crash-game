...
export interface ConfirmWalletDebitInput {
  betId: string;
}

export interface ConfirmWalletDebitOutput {
  betId: string;
  status: string;
}

export class ConfirmWalletDebitUseCase {
  constructor(
  ...
  async execute(
    input: ConfirmWalletDebitInput,
  ): Promise<ConfirmWalletDebitOutput> {
    ...
    const bet = round.confirmBetDebit(input.betId, this.clock.now());

    await this.roundRepository.save(round);

    const snapshot = bet.toSnapshot();

    this.realtimeNotifier.notifyBetAccepted({
      id: snapshot.id,
